...
class CardPaymentCtrl {
  static initiatePayment = asyncWrapper(async (req, res, next) => {
    ...
    if (response?.status === 'error') {
      ...
      if (response.meta.authorization.mode === 'pin') {
        ...
        req.session.custom_payload.paymentPayload = req.session.paymentPayload;
        ...
        logger.error('Failed to Complete Transaction: Reason: Card type is not supported.', {
  ...
  static cardAuthorization = asyncWrapper(async (req, res, next) => {
    ...
    if (Object.keys(payload).length === 0 || Object.keys(custom_payload).length === 0)
      return next(
        createCustomError(
    ...
    switch (response?.meta?.authorization?.mode) {
      ...
      default:
        ...
        if (transaction.data.status == 'successful') {
          ...
          const payment_name = req.session.custom_payload.payment_name;
          ...
          if (!isLogged) {
            logger.error(
              `Failed to create Transaction record. | Amount ${formatCurrency(
                amount
              )} | Transaction reference: ${response.data.tx_ref} | Transaction code: ${
                response.data.id
              } `,
              {
                module: 'cardPaymentController.js',
          ...
          const subscriptionNames = [TIIZA_REAL, TIIZA_REAL_PLUS, TIIZA_LITE, TIIZA_LITE_PLUS];
          // dynamic data
          if (subscriptionNames.includes(payment_name)) {
            await registerLostItem(req, res, next, custom_payload, txInfo);
  ...
  static validateCardTransaction = asyncWrapper(async (req, res, next) => {
    const requestId = res.getHeader('X-request-Id');
    ...
    if (Object.keys(requestPayload).length === 0 || Object.keys(custom_payload).length === 0)
      return next(
        createCustomError(
          'sorry,You do not have the necessary information to access this resource',
          403
    ...
    if (response?.data?.status === 'successful' || response?.data?.status === 'pending') {
      ...
      if (transaction.data.status == 'successful') {
        ...
        const commission = parseFloat(req.session.custom_payload.commission);
        const payment_name = req.session.custom_payload?.paymentPayload.payment_name;
        // return
        const isLogged = await TransactionLogModel.create({
          customer_id: user_id,
        ...
        if (!isLogged) {
          logger.error(
            `Failed to create  transaction record |  Amount:  ${formatCurrency(
              txInfo.amount
            )}. Transaction reference:${txInfo.tx_ref}, Transaction code ${txInfo.id}`,
            {
              module: 'cardPaymentController.js',
              userId: req.user ? req.user.user_id : null,
              requestId: requestId,
...
const registerLostItem = async (req, res, next, custom_payload, txInfo) => {
  const payment_name = req.session.custom_payload?.paymentPayload.payment_name;
  const itemInfo = custom_payload.paymentPayload;
  itemInfo.lost_date = new Date(itemInfo.lost_date);
  ...
  if (!isSubscribed) {
    ...
    return next(
      createCustomError('System is unable to create subscription.Please contact support Team', 500)
    );
  }
...
const bookQrCode = async (req, res, next, txInfo) => {
  const { user_id } = req.user;
  const { payment_name, amount, quantity, address } = req.session.custom_payload?.paymentPayload;
