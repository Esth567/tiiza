...
const fetchQRCodeOrdersCtrl = asyncWrapper(async (req, res, next) => {
  const qrCodeBookings = await BookingModel.findAll({
    include: {
      ...
      attributes: {
        exclude: ['password', 'user_role', 'createdAt', 'updatedAt', 'is_verified', 'user_id'],
      },
    },
  });

  if (qrCodeBookings.length == 0) return next(createCustomError('No Data found', 404));

  return res.status(200).json({ success: true, payload: qrCodeBookings });
});
