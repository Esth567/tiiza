...
const { EmailValidator, PasswordValidator } = require('../../validation/validation');
...
const resetPasswordCtrl = asyncWrapper(async (req, res, next) => {
  ...
  await sendMail(options, emailTemplate);

  res.status(200).json({ success: true, payload: 'Password reset email sent' });
});
// ===============================||  CONFIRM RESET PASSWORD ||===============================

const confirmResetPasswordCtrl = asyncWrapper(async (req, res, next) => {
  ...
  if (newPassword !== confirmPassword) return next(createCustomError('password Do not match', 400));
  ...
  try {
    ...
    console.log(error);
    return next(
      createCustomError('Sorry, Your reset link has expired.Please request for a new link', 400)
    );
  }
