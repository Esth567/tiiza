...
function logPendingTransaction(transaction, res) {
    const timestamp = new Date().toISOString().replace('T', ' ').replace(/\..+/, '');
    const logMessage = `${timestamp}, | Transaction Type:${transaction.transaction_type}   | Transaction Ref: ${transaction.tx_ref} | Customer ID: ${transaction.user_id} | Status: ${transaction.status} | Amount: ${transaction.amount}\n`;
