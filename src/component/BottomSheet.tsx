...
function BottomSheet({ children, show, height = DEFAULT_HEIGHT, onOuterClick }: Props) {
  ...
  return (
    <>
      ...
      {show && (
        <Pressable onPress={onOuterClick} style={[style.outerOverlay, { height: screenHeight }]}>
          <View />
        </Pressable>
      )}
      <Animated.View style={[style.bottomSheet, { height, bottom }]}>{children}</Animated.View>
    </>
  );
}
